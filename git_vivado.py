...
def main():
    ...
    default_xpr_path = os.path.abspath(
        os.path.join(script_dir, "..", "proj", "%s.xpr" % project_name)
    )
    default_version = config_settings["VivadoVersion"]

    # Parse SYS.ARGV
    parser = argparse.ArgumentParser(
        description="Handles vivado project git repo operations"
    )
    parser.add_argument(
        "-f",
        dest="force",
    ...
    if hasattr(args, "repo_path"):
        funcargs["repo_path"] = os.path.abspath(
            os.path.join(os.getcwd(), args.repo_path)
        )
    ...
    if hasattr(args, "version"):
        funcargs["vivado_cmd"] = os.path.join(
            os.path.abspath(config_settings["VivadoInstallPath"]),
            args.version,
            "bin",
            "vivado",
        )
...
def do_checkin(args):
    ...
    script_path = os.path.join(
        args["script_dir"], "digilent_vivado_checkin.tcl"
    ).replace("\\", "/")
    xpr_path = args["xpr_path"].replace("\\", "/")
    repo_path = args["repo_path"].replace("\\", "/")
    version = args["version"].replace("\\", "/")

    if not args["force"] and not accept_warning(
        "Files and directories contained in %s may be overwritten. Do you wish to continue?"
        % repo_path
    ):
        sys.exit()

    print(
        "Checking in project %s to repo %s"
...
def do_checkout(args):
    ...
    vivado_cmd = args["vivado_cmd"].replace("\\", "/")
    script_path = os.path.join(
        args["script_dir"], "digilent_vivado_checkout.tcl"
    ).replace("\\", "/")
    xpr_path = args["xpr_path"].replace("\\", "/")
    repo_path = args["repo_path"].replace("\\", "/")
    version = args["version"].replace("\\", "/")
    ...
    if not args["force"] and not accept_warning(
        "Files and directories contained in %s may be overwritten. Do you wish to continue?"
        % os.path.dirname(xpr_path)
    ):
        sys.exit()

    print(
        "Checking out project %s from repo %s"
        % (os.path.basename(xpr_path), os.path.basename(repo_path))
    )
    notrace = "" if DEBUG_VIVADO_TCL_TRACE else " -notrace"
